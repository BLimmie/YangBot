...
>>>>>>> 1a8995c20b3d17a52b608a523d354203b4e65b39
...
<<<<<<< HEAD
    ...
    def state(self, new_state):
        raise AttributeError("Can't set state, please use update_state") # Prevents setting a property without the right function
=======
...
class State:
    ...
    ## Raises
      `KeyError`: Embed_dict passed has a key that isn't in an embed_info
=======
class state:
    '''
    An object representing a state for a machine. Behaves like a dictionary for its data, like an object for its embed (see Behavior).
...
<<<<<<< HEAD
        ...
        for key in embed_dict.keys():
            if key not in self.embed_info.keys():
                raise KeyError("Invalid key in dictionary.")
            else:
                self.embed_info[key] = embed_dict[key]
